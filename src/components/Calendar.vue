<template>
    <div class="calendar flex flex-col bg-white bg-opacity-80 dark:bg-gray-600 dark:bg-opacity-50 backdrop-filter backdrop-blur-lg rounded-md shadow-md duration-200 overflow-hidden" :class="{ 'show': isOpen }">
        <div class="flex items-center bg-black bg-opacity-5 px-6 py-4">
            <span class="text-sm text-gray-700 dark:text-white flex-grow">{{ currentDate.format('jYYYY/jM/jD') }}</span>
        ...
        <div class="flex justify-center items-center gap-3 px-6 pt-4">
            <span class="text-gray-700 dark:text-white text-sm flex-grow">{{ this.date.format('jMM') + ' ' + this.date.format('jYYYY') }}</span>
...
export default {
    ...
    methods: {
        ...
        addMonth() {
            this.date.add('1', 'jMonth')
            this.updateTime()
        },
        subtractMonth() {
            this.date.subtract('1', 'jMonth')
            this.updateTime()
        },
        updateTime() {
            this.weekDays = this.date._locale.__weekdaysMin
            const firstDay = this.date.startOf('jMonth').day()
            ...
            for (let i = 0; i < 42; i++) {
                if (i < firstDay) {
                    daysObject[i] = {
                        day: this.date.clone().subtract((firstDay - i), 'days').format('jD'),
                        state: -1,
                        today: false
                    }
                    ...
                    daysObject[i] = {
                        day: i - firstDay + 1,
                        state: 0,
                        today: this.currentDate.jDate() === (i - firstDay + 1) && this.currentDate.jMonth() === this.date.jMonth()
                    }
                } else {
                    daysObject[i] = {
                        day: this.date.clone().add((i - daysNumber ), 'days').format('jD'),
                        state: 1,
                        today: false
                    }
                }
